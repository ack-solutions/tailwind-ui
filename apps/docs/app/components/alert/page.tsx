...
export default function AlertPlaygroundPage() {
  ...
  const code = useMemo(() => {
    const props: string[] = [];
    if (tone !== "soft") props.push(`tone=\"${tone}\"`);
  ...
  }, [tone, color, withIcon, title, desc]);

  return (
    <div>
      <h1 className="text-2xl font-semibold">Alert</h1>
      <p className="ack-muted mt-2 max-w-2xl">Play with tone, color, title, icon, and description.</p>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <section className="border rounded p-4">
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-sm ack-muted mb-1">Title</label>
              <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border rounded px-3 h-9" />
            </div>
            <div>
              <label className="block text-sm ack-muted mb-1">Description</label>
              <textarea value={desc} onChange={(e) => setDesc(e.target.value)} className="w-full border rounded px-3 py-2 min-h-[90px]" />
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-sm ack-muted mb-1">Tone</label>
                <select value={tone} onChange={(e) => setTone(e.target.value as Tone)} className="w-full border rounded px-2 h-9">
        ...
        <section className="lg:col-span-2">
          <div className="border rounded p-6 bg-surface">
            <Alert tone={tone} color={color} title={title || "Update available"} icon={withIcon ? <InfoIcon /> : undefined}>
